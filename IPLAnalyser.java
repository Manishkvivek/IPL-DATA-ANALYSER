...
//					.thenComparing(IPL_Bowling_CSV::getSr);
	...
	public String returnsMaxHundredsScorerAndBestAveragePlayer(String csvFilePath) throws CSVException, IPLException {
		try (Reader reader = Files.newBufferedReader(Paths.get(csvFilePath));) {
			ICSVBuilder csvBuilder = CSVBuilderFactory.createCSVBuilder();
			IPLBatsmanList = csvBuilder.returnsListToTheLoadingFunction(reader, IPL_Batsman_CSV.class);

			Comparator<IPL_Batsman_CSV> iplComparator = Comparator.comparing(IPL_Batsman_CSV::getHundreds)
					.thenComparing(IPL_Batsman_CSV::getAvg);
